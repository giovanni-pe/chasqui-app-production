...
async googleSignIn(idToken: string): Promise<AuthResponse> {
  const { success, data, message } =
    await api.post<AuthResponse>('/auth/google', { id_token: idToken })

  if (!success || !data) throw new Error(message || 'Error en Google Sign-In')
  return data
},
 async completeProfile(payload: CompleteProfilePayload): Promise<User> {
    // ⇢ nuevo endpoint
    const { success, data, message } =
      await api.post<User>('/auth/complete-profile', payload)

    if (!success || !data)
      throw new Error(message || 'Error al completar perfil')

    return data            // retorna el perfil fresco desde backend
  },
